...
class ResponseFactory extends EventEmitter {
  ...
  attachHandlers() {
    this.game.on('created', () => {
      const payload = this.constructor.forLoad(this.game.master, this.game)
      this.send(this.game.master, 'load', payload)
      this.sendToOthers(this.game.master, 'redirectToGame', this.game.path)
    })
