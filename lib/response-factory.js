...
class ResponseFactory extends EventEmitter {
  ...
  sendToAllBut(userIds, type, payload = null) {
    this.game.players.forEach((user) => {
  ...
  sendToOthers(userId, type, payload = null) {
    this.sendToAllBut([userId], type, payload)
  }
  ...
  attachHandlers() {
    ...
    this.game.on('playerRemoved', (userId) => {
      ...
      this.sendToAllBut([this.game.master, userId], 'removePlayer', userId)
    })

    this.game.on('playerLeft', (userId) => {
      this.send(userId, 'leaveGame')
      this.sendToOthers(userId, 'removePlayer', userId)
    })
    ...
    this.game.on('nextEntry', () => {
      this.send(this.game.activePlayer, 'nextEntry', this.game.activeEntry)
    })
